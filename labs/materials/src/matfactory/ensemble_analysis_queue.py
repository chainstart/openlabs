"""Persistent waiter for the matched LLZTO production-ensemble analysis."""
...
def _verify_report(path: Path, protocol_sha256: str) -> dict[str, Any]:
    ...
    if stored != fingerprint(unsigned):
        raise RuntimeError(f"ensemble report fingerprint mismatch: {path}")
    if report.get("protocol_sha256") != protocol_sha256:
        raise RuntimeError("existing ensemble report uses a different protocol")
    return report


def run_ensemble_analysis_queue(
    protocol_path: Path | str,
    *,
    state_path: Path | str,
    poll_seconds: float = 30.0,
) -> dict[str, Any]:
    """Wait for both matched runs and build exactly one analysis report."""
    ...
    if protocol.get("schema_version") != "1.0":
        raise ValueError("ensemble protocol schema_version must be '1.0'")
    prerequisites = []
    locked_paths = [
        source,
    ...
    for role in ("reference", "comparison"):
        config = protocol[role]
        campaign_protocol = _repo_path(config["campaign_protocol_path"])
        locked_paths.append(campaign_protocol)
        prerequisites.append(
            _repo_path(config["campaign_root"]) / config["run_id"] / "result.json"
        )
    locked_files = [
    ...
    config_record = {
        ...
        "prerequisite_result_paths": [str(path) for path in prerequisites],
    ...
    if output_state.is_file():
        ...
        if state.get("queue_fingerprint") != queue_fingerprint:
            raise RuntimeError(
                f"ensemble analysis queue configuration changed: {output_state}"
            )
    else:
        state = {
            "schema_version": "1.0",
    ...
    def verify_locks() -> None:
        for record in locked_files:
            if sha256_file(record["path"]) != record["sha256"]:
                raise RuntimeError(
                    f"ensemble analysis locked file changed: {record['path']}"
                )

    destination = Path(config_record["output_path"])
    try:
        while not all(path.is_file() for path in prerequisites):
            ...
            _update(
                output_state,
                state,
                "waiting_for_matched_ensemble_runs",
                waiting={
        ...
        verify_locks()
        _update(output_state, state, "building_ensemble_sensitivity")
        if destination.is_file():
            report = _verify_report(destination, protocol_sha)
        else:
            report = build_ensemble_sensitivity_report(source)
            atomic_write_json(destination, report)
            if _verify_report(destination, protocol_sha) != report:
                raise RuntimeError("stored ensemble report differs from builder output")
