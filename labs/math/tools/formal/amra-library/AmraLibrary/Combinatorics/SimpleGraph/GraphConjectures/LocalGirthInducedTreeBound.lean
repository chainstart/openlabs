...
/-!
# Local-girth bound for induced trees

*Reference:*
[E. DeLaVina, Written on the Wall II, Conjectures of Graffiti.pc](http://cms.dt.uh.edu/faculty/delavinae/research/wowII/)
-/

namespace WrittenOnTheWallII.LocalGirthInducedTreeBound
...
/--
Strengthened local-girth bound.

For a simple connected finite graph `G`, the largest induced tree has at least
`max_v l(v) + max(1, girth(G)-3)` vertices. For acyclic and small-girth graphs
the star bound supplies the `+1` term; for girth at least four the neighbourhood
of a maximum-degree vertex is independent and can be joined to a sufficiently
...
theorem local_girth_induced_tree_bound (G : SimpleGraph α) (h : G.Connected) :
      ...
      · constructor
        ...
        · intro u c hc
          ...
          · have hsnd_ne : c.snd.1 ≠ v := by
            ...
            exact no_cycle_at_leaf (c.rotate hmem) (hc.rotate hmem) hsnd_ne
          · exact no_cycle_at_leaf c hc huv
  by_cases hsmall : Nat.max 1 (G.girth - 3) ≤ 1
  · have hLpos : 0 < largestInducedTreeSize G := by
      have := hstar (Classical.choice inferInstance)
      omega
    have hsup_lt :
  ...
  · have hnotacyc : ¬ G.IsAcyclic := by
      ...
      exact hsmall (by rw [hg0]; norm_num)
    have hgsub_gt : 1 < G.girth - 3 := by
      by_contra hle_not
      have hle : G.girth - 3 ≤ 1 := Nat.le_of_not_gt hle_not
      exact hsmall (Nat.max_le.mpr ⟨by omega, hle⟩)
    have hg5 : 5 ≤ G.girth := by omega
    have htriangleFree_of_girth : 4 ≤ G.girth → G.CliqueFree 3 := by
      intro hg t ht
      obtain ⟨_u, w, hwc, hlen⟩ :=
    ...
    have hindep_neighbor_of_girth :
      ...
      intro hg v
      exact G.isIndepSet_neighborSet_of_triangleFree (htriangleFree_of_girth hg) v
    have hpath_long : ∀ v, ∃ w, ∃ p : G.Walk v w, p.IsPath ∧ G.girth - 3 ≤ p.length := by
      intro v
      obtain ⟨a, c, hc, hclen⟩ := SimpleGraph.exists_girth_eq_length.mpr hnotacyc
      let S : Finset α := c.support.toFinset
      have hS_nonempty : S.Nonempty := by
        refine ⟨a, ?_⟩
        dsimp [S]
        exact List.mem_toFinset.mpr c.start_mem_support
      obtain ⟨z, hzS, hzmin⟩ := Finset.exists_min_image S (fun x => G.dist v x) hS_nonempty
      have hzmem : z ∈ c.support := by
        dsimp [S] at hzS
        exact List.mem_toFinset.mp hzS
      obtain ⟨pv, hpv_path, hpv_len⟩ := h.exists_path_of_dist v z
      have hpv_cycle_only : ∀ x, x ∈ pv.support → x ∈ c.support → x = z := by
        intro x hxpv hxc
        obtain ⟨i, hix, hi⟩ := SimpleGraph.Walk.mem_support_iff_exists_getVert.mp hxpv
        by_cases hxz : x = z
        · exact hxz
        have hi_lt : i < pv.length := by
          have hi_ne : i ≠ pv.length := by
            intro hil
            apply hxz
            rw [← hix]
            exact (hpv_path.getVert_eq_end_iff hi).mpr hil
          omega
        have hdist_le_i : G.dist v x ≤ i := by
          rw [← hix]
          calc
            G.dist v (pv.getVert i) ≤ (pv.take i).length := SimpleGraph.dist_le (pv.take i)
            _ = i := by
              rw [SimpleGraph.Walk.take_length]
              exact Nat.min_eq_left hi
        have hz_le_x : G.dist v z ≤ G.dist v x := by
          exact hzmin x (by dsimp [S]; exact List.mem_toFinset.mpr hxc)
        omega
      let cr : G.Walk z z := c.rotate hzmem
      have hcr_cycle : cr.IsCycle := hc.rotate hzmem
      have hcr_len : cr.length = G.girth := by
        dsimp [cr]
        rw [SimpleGraph.Walk.rotate, SimpleGraph.Walk.length_append]
        have hspec := congrArg SimpleGraph.Walk.length (SimpleGraph.Walk.take_spec c hzmem)
        rw [SimpleGraph.Walk.length_append] at hspec
        omega
      let r := G.girth - 3
      let qs : G.Walk z (cr.getVert r) := cr.take r
      have hr_le_cr : r ≤ cr.length := by dsimp [r]; omega
      have hqs_len : qs.length = r := by
        dsimp [qs]
        rw [SimpleGraph.Walk.take_length]
        exact Nat.min_eq_left hr_le_cr
      have hqs_path : qs.IsPath := by
        apply (SimpleGraph.Walk.IsPath.getVert_injOn_iff qs).mp
        intro i hi j hj hij
        have hi_cr : i ≤ cr.length - 1 := by
          rw [hcr_len]
          dsimp [qs, r] at hi
          rw [SimpleGraph.Walk.take_length, Nat.min_eq_left hr_le_cr] at hi
          omega
        have hj_cr : j ≤ cr.length - 1 := by
          rw [hcr_len]
          dsimp [qs, r] at hj
          rw [SimpleGraph.Walk.take_length, Nat.min_eq_left hr_le_cr] at hj
          omega
        apply hcr_cycle.getVert_injOn' hi_cr hj_cr
        dsimp [qs] at hij
        rw [SimpleGraph.Walk.take_getVert, SimpleGraph.Walk.take_getVert] at hij
        have hi_le_r : i ≤ r := by
          dsimp [qs] at hi
          rw [SimpleGraph.Walk.take_length, Nat.min_eq_left hr_le_cr] at hi
          exact hi
        have hj_le_r : j ≤ r := by
          ...
          rw [SimpleGraph.Walk.take_length, Nat.min_eq_left hr_le_cr] at hj
          exact hj
        simpa [Nat.min_eq_right hi_le_r, Nat.min_eq_right hj_le_r] using hij
      have hdisj : ∀ x, x ∈ pv.support → x ∈ qs.support.tail → False := by
        intro x hxpv hxqs
        have hxqs_supp : x ∈ qs.support := List.mem_of_mem_tail hxqs
        have hxcr : x ∈ cr.support := by
          dsimp [qs] at hxqs_supp
          rw [SimpleGraph.Walk.take_support_eq_support_take_succ] at hxqs_supp
          exact List.mem_of_mem_take hxqs_supp
        have hxc : x ∈ c.support := by
          rwa [SimpleGraph.Walk.mem_support_rotate_iff] at hxcr
        have hxz' : x = z := hpv_cycle_only x hxpv hxc
        subst x
        have hz_qs_tail : z ∈ qs.support.tail := hxqs
        have hnodup := hqs_path.support_nodup
        rw [← List.cons_head_tail qs.support_ne_nil] at hnodup
        simp only [SimpleGraph.Walk.head_support] at hnodup
        exact (List.nodup_cons.mp hnodup).1 hz_qs_tail
      let p : G.Walk v (cr.getVert r) := pv.append qs
      have hp_path : p.IsPath := by
        rw [SimpleGraph.Walk.isPath_def]
        rw [SimpleGraph.Walk.support_append]
        apply List.Nodup.append
        · exact hpv_path.support_nodup
        · exact hqs_path.support_nodup.tail
        · intro x hx1 hx2
          exact hdisj x hx1 hx2
      have hp_len : p.length = G.dist v z + r := by
        dsimp [p]
        rw [SimpleGraph.Walk.length_append, hpv_len, hqs_len]
      refine ⟨cr.getVert r, p, hp_path, ?_⟩
      rw [hp_len]
      omega
    have hneighbor_eq_card :
        ∀ v, indepNeighborsCard G v = Fintype.card (G.neighborSet v) := by
      ...
      · have hInd : (G.induce (G.neighborSet v)).IsIndepSet
        ...
        simpa using SimpleGraph.IsIndepSet.card_le_indepNum hInd
    have hlarge_tree :
        ∀ v, (G.girth - 3) + Fintype.card (G.neighborSet v) ≤
          largestInducedTreeSize G := by
      intro v
      let r := G.girth - 3
      have hlong_path : ∃ w, ∃ p : G.Walk v w, p.IsPath ∧ r ≤ p.length := by
    ...
    obtain ⟨v, _hv, hv⟩ := Finset.exists_mem_eq_sup (Finset.univ : Finset α)
      (Finset.univ_nonempty) (indepNeighborsCard G)
    have hmax_eq : Nat.max 1 (G.girth - 3) = G.girth - 3 := by
      exact Nat.max_eq_right (by omega)
    rw [hv, hneighbor_eq_card v, hmax_eq]
...
/--
WOWII Conjecture 141 follows formally from the local-girth strengthening.
-/
theorem conjecture141_from_local_girth_strong (G : SimpleGraph α) (h : G.Connected) :
  ...
  have harith : (G.girth + 1) / 2 - 1 ≤ Nat.max 1 (G.girth - 3) := by
    ...
    · have hg5 : 5 ≤ G.girth := by omega
      have hmax : Nat.max 1 (G.girth - 3) = G.girth - 3 := by
        exact Nat.max_eq_right (by omega)
      rw [hmax]
      have hdiv : (G.girth + 1) / 2 ≤ G.girth - 2 := by
        apply Nat.div_le_of_le_mul
        omega
      omega
  have hsum :
      (G.girth + 1) / 2 - 1 + (Finset.univ.sup (indepNeighborsCard G)) ≤
      (Finset.univ.sup (indepNeighborsCard G)) + Nat.max 1 (G.girth - 3) := by
    rw [Nat.add_comm ((G.girth + 1) / 2 - 1)
      (Finset.univ.sup (indepNeighborsCard G))]
    exact Nat.add_le_add_left harith (Finset.univ.sup (indepNeighborsCard G))
  exact hsum.trans hstrong
...
end WrittenOnTheWallII.LocalGirthInducedTreeBound
