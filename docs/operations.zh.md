...
`init` 也会把现有本地 v2 SQLite 以加列方式迁移到 v3；不会删除 campaign、task 或研究索引。
...
论文就绪审查任务；该任务使用 `frontier` 档，但仍不产生任何外部发布副作用。

`tick` 是一次短生命周期、幂等调度，不是永不退出的大 Agent。它先接收完成结果，再回收
过期租约，最后按并发上限启动新任务。单个任务失败、等待人工或被隔离，不会阻塞其他
campaign。
...
每个 campaign 同时只运行一个任务。默认单任务最多 4 小时、每个 campaign 累计 Agent
运行时间最多 24 小时；耗尽后该 campaign 的排队任务转为 `NEEDS_HUMAN`，不会占用其他
campaign 的并发槽。
...
研究者、同一实验执行链和同一稿件修订链可按 session ID 恢复，但每个节点仍启动一个短进程
并在完成后退出。独立复现实验、`needs_replan`、从研究切换到写作，以及所有 reviewer 都启动
...
真实 24 小时 soak run，检查：无重复 attempt、无跨 campaign 入库、无超过并发/预算的启动、
